<template>
    <div class="tag_list">
        ...
        <input v-if="input" type="text" v-model="newTagName" list="existingTags" @keyup.enter="addTag()">
        <datalist id="existingTags">
            <option v-for="(tag, index) in existingTags" :key="`existing_tag_${index}`" :value="tag.name" />
        </datalist>
    </div>
